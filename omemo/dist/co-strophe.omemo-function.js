...
(function () {
    Strophe.addConnectionPlugin('omemo', function () { 
        let _jid, _id, connection
        let signal_store

        let init, setup, arm, gen_prekeys, refresh_prekeys
        let on_device, on_bundle, on_message, on_headline
        let publish_device, publish_bundle
        let refresh_bundle, new_id
        ...
        init = function (conn) {
            ...
            publish = connection.pep.publish
        ...
        setup = function (jid) {
            ...
            try { 
                if (SignalProtocolStore) {
                    connection._signal_store = new SignalProtocolStore(jid, id)
                } 
            } catch(e) { 
            ...
            try { 
                if (OmemoStore) {
                    connection._omemo_store = new OmemoStore(jid, id) //probably not needed
                } 
            } catch(e) { 
                throw new Error ("SignalProtocolStore not found")
            }

            Promise.all([arm()]).then(e => {
                console.log("armed") 
        ...
        arm = function () {
            ...
            Promise.all([
                ...
                registrationId = _id
                connection._signal_store.put('registrationId', registrationId)
                connection._signal_store.put('identityKey', result[0])
                connection._signal_store.getIdentityKeyPair().then((ikey) =>
                    kh.generateSignedPreKey(ikey, 1)).then((skey) => {
                        ...
                        connection._signal_store.storeSignedPreKey(1, key)
                        publish_device()                                         
                        publish_bundle()                                         
                    })
                _address = new libsignal.SignalProtocolAddress(_jid, connection._signal_store.get('registrationId'));
            }).then( o => { 
                connection._signal_store.setLocalStore(_jid, _id)
            })
        ...
        on_headline = function (stanza) {
            let ctxt = this
            if(is_device(stanza)) {
                print("calling on device")
                print(on_device)
                on_device(stanza)
            }
            else if(is_bundle(stanza)) {
                on_bundle(stanza)
            }
            return true 
        }
        ...
        fetch_bundle = function (to, device) {
            let ctxt = this
            let res = $iq({type: 'get', from: _jid, to: to, id: 'fetch1'})
        ...
        publish_bundle= function () {
            ...
            let promises = [
                connection._signal_store.loadSignedPreKey(sk_id),
                connection._signal_store.getIdentityKeyPair(),
                connection._signal_store.loadSignedPreKeySignature(sk_id),
                connection._signal_store.getLocalRegistrationId()
            ]
            return Promise.all(promises).then(o => {
                ...
                let res = $iq({type: 'set', from: _jid, id: 'anounce2'})
                    .c('pubsub', {xmlns: 'http://jabber.org/protocol/pubsub'})
                    .c('publish', {node: Strophe.NS.OMEMO_BUNDLES + ":" + _id})
                    .c('item')
                ...
                let keys = connection._signal_store.getPreKeyBundle(ctxt)
        ...
        publish_device = function () {
            ...
            list.c('device', {id: _id})
            publish(Strophe.NS.OMEMO_DEVICELIST, [{
                data: list.tree()
            }], null)
        }
        genPreKeys = function (range) {
            ...
            for (let i = 3; i < range + 1; i++ ) {
                promises.push(kh.generatePreKey(i).then((k) =>  {
                    ...
                    connection._signal_store.storePreKey(i,key)
                }))
            }
...
function is_device (stanza)  {
    return $(stanza).find("list").length != 0
}
function is_bundle (stanza)  {
